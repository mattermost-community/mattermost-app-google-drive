...
import { oAuth2Connect } from '../forms/oauth-google';
import { AppCallRequest, AppCallResponse, Oauth2App } from '../types';
 import { CallResponseHandler, newOKCallResponseWithData, newOKCallResponseWithMarkdown } from "../utils/call-responses";
...
export const getConnectGoogleURL: CallResponseHandler = async (req: Request, res: Response) => {
   ...
   const message: string = isConnected(oauth2)
      ? `You are already logged into Google with user ${oauth2.user?.user.name}`
